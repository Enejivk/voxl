## Voxl - Voice to Content App

### Current State (April 2026)
Voxl is a voice-to-content application that allows users to record audio and convert it to LinkedIn posts, tweets, emails, and other formats using AssemblyAI for speech-to-text transcription.

### Speech-to-Text Integration (AssemblyAI)
The app uses AssemblyAI for transcription. Key files:

**Server Module** (`/user-app/src/server/transcription/index.ts`):
- `configSchema.assemblyAiApiKey` - Secret config for API key (set in Modelence dashboard)
- `mutations.uploadAudio` - Uploads base64 audio to AssemblyAI
- `mutations.startTranscription` - Starts transcription job
- `mutations.getTranscriptionResult` - Polls for transcription status/result

**Results Page** (`/user-app/src/client/pages/TranscriptionResultPage.tsx`):
- Displays transcribed text with format badge
- Copy to clipboard functionality
- Character/word count
- "Record Again" navigation

**Home Page Recording Flow**:
1. User clicks mic button to start recording
2. MediaRecorder API captures audio
3. Visual feedback: red theme, faster animations, timer
4. On stop: audio is processed through transcription pipeline
5. Results navigate to `/result` page with transcribed text

**Configuration Required**:
- Set `transcription.assemblyAiApiKey` in Modelence Config dashboard

### Landing Page Design
The home page (`/`) features a premium dark theme with purple accents and framer-motion animations:

**Key Sections:**
- Fixed navbar with logo, pricing/privacy links, and sign-in button
- Hero with animated recording circle, concentric rings, and waveform visualizations
- Format selector bar (FileText, MessageSquare, X, LinkedIn, Mail)
- "Voice to Content in 3 Steps" section with feature cards
- "Built for People Who Think Out Loud" section with audience cards (Founders, Creators, Professionals)
- "Why Yapir?" features grid with various card layouts
- Animated FAQ accordion using framer-motion
...
### Content Sharing Feature (April 2026)
After transcription completes, users see the result page with integrated platform sharing:

**TranscriptionResultPage** (`/user-app/src/client/pages/TranscriptionResultPage.tsx`):
- **Original Notes section** - Shows the raw transcription at the top with copy button
- **Platform Switcher** - Buttons to switch between X, LinkedIn, and Email previews
- **Platform Previews** - Realistic preview cards for each platform:
  - X Post Card: Dark theme, verified badge, engagement stats, 280 char limit indicator
  - LinkedIn Post Card: White theme with professional styling
  - Email Preview Card: Email format with header, body, and signature
- **Action Buttons** - Copy text and platform-specific share buttons (Post to X, Share on LinkedIn, Open in Email)
- Auto-formats content for X using AI when page loads

**Content Module** (`/user-app/src/server/content/index.ts`):
- `mutations.formatForX` - Uses @modelence/ai to format text for X posts
- `mutations.formatForLinkedIn` - Formats text for LinkedIn posts
- `mutations.regenerateWithTone` - Regenerates content with different tones (storytelling, emotional, thought_leadership, curiosity, inspirational)
- `mutations.adjustLength` - Makes posts longer or shorter
- Uses OpenAI gpt-4o-mini model via Modelence AI integration
- Prompt optimized for engaging, shareable content under 280 chars
- **Rate Limiting**: Global app limit of 200 posts per day shared by all users
  - Tracked via `globalUsage` Store (date + count)
  - `getUsage` query returns current global usage count for display
  - Shows progress bar in header with "X/200 used"
  - Throws error "Daily app limit reached" when exceeded

**Dependencies Added**:
- @modelence/ai - Modelence AI SDK for text generation

**AI Integration Required**:
- OpenAI integration must be configured in Modelence Cloud AI Integrations dashboard

### File Structure
```
src/client/pages/
├── HomePage.tsx              # Main landing page with recording functionality
├── TranscriptionResultPage.tsx  # Results with notes + platform switcher + previews
├── LoginPage.tsx
├── SignupPage.tsx
└── ...

src/server/
├── app.ts                    # App entry (includes transcription, content modules)
├── example/                  # Example module
├── transcription/
│   └── index.ts              # AssemblyAI transcription module
└── content/
    └── index.ts              # Content formatting module (AI-powered)
```
